...
votes <- primary %>%  #get the winners and the fraction of votes the won
            ...
            summarize(winner = candidate[which.max(fraction_votes)],
                                            Vote = max(fraction_votes), votes = max(votes))
...
To get a better understanding I will use the really cool `ggpairs` function to plot counties based on different demographic features and color them based on who won. Then we will zoom in and look at more specific plots that look interesting. 


```{r, message= FALSE, warning= FALSE, out.width = "1300px" }
ggpairs(data = votes[,c(3, 5, 7, 8)], colour = "winner")
```

Whoa information overload. Let's take a look at some individual plots and see what we get:
